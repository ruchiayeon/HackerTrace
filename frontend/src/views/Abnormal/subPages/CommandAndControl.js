...
function CommAControl() {
  ...
  const [commAConDatas, setCommAConDatas] = useState(false);
  ...
  const tableAxiosData = async(startDate, endDate, selectColum, search, selectHostIp) => {
    ...
    try{
      ...
      setCommAConDatas(response.data.data);
      console.log(response.data.data);
      ...
      setError(e);
      console.log(e)
      if(!commAConDatas) return <div>일치하는 데이터가 없습니다.</div>;
    }
    //로딩 실패시 flag를 달아서 이동
    setLoading(false);
  };
