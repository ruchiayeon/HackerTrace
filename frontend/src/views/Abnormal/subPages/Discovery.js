...
function Discovery() {
  ...
  const [discoveryDatas, setDiscoveryDatas] = useState(false);
    ...
    try{
      ...
      if(pageNumbers>1){
        for(let i =0; i<1000; i++){
          discoveryDatas.push(response.data.data[i])
        }
      }else{
        setDiscoveryDatas(response.data.data);
      }
  ...
  if(!discoveryDatas) return submitValue()
            ...
            <CCardBody>
              <Clock/>
                <CRow className="searchtoolbar"> 
                  ...
                  <CDataTable
                    items={discoveryDatas}
                    fields={fields}
                    itemsPerPage= {10}
                    hover
                    pagination
                    onPageChange={(index) => {
                      if(index === discoveryDatas.length/10){
