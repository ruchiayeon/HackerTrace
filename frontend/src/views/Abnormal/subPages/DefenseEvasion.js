...
function DefenseEvasion() {
  ...
  const [defenseEvDatas, setDefenseEvDatas] = useState(false);
    ...
    try{
      ...
      if(pageNumbers>1){
        for(let i =0; i<1000; i++){
          defenseEvDatas.push(response.data.data[i])
        }
      }else{
        setDefenseEvDatas(response.data.data);
      }
     
    }catch(e){
      //에러시 flag를 달아서 이동
      setError(e);
      if(!defenseEvDatas) return <div>일치하는 데이터가 없습니다.</div>;
    }
    //로딩 실패시 flag를 달아서 이동
    setLoading(false);
  };
  ...
  if(!defenseEvDatas) return submitValue()
            ...
            <CCardBody>
              <Clock/>
                <CRow className="searchtoolbar"> 
                  ...
                  <CDataTable
                    items={defenseEvDatas}
                    fields={fields}
                    itemsPerPage= {10}
                    hover
                    pagination
                    onPageChange={(index) => {
                      if(index === defenseEvDatas.length/10){
