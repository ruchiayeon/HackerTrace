...
function CreditAccess() {
  ...
  const [creaditAccessDatas, setCreaditAccessDatas] = useState(false);
    ...
    try{
      ...
      if(pageNumbers>1){
        for(let i =0; i<1000; i++){
          creaditAccessDatas.push(response.data.data[i])
        }
      }else{
        setCreaditAccessDatas(response.data.data);
      }
  ...
  if(!creaditAccessDatas) return submitValue()
            ...
            <CCardBody>
              <Clock/>
                <CRow className="searchtoolbar"> 
                  ...
                  <CDataTable
                    items={creaditAccessDatas}
                    fields={fields}
                    itemsPerPage= {10}
                    hover
                    pagination
                    onPageChange={(index) => {
                      if(index === creaditAccessDatas.length/10){
