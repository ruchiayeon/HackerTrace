...
function LateralMovement() {
  ...
  const [lateralDatas, setLateralDatas] = useState(false);
    ...
    try{
      ...
      if(pageNumbers>1){
        for(let i =0; i<1000; i++){
          lateralDatas.push(response.data.data[i])
        }
      }else{
        setLateralDatas(response.data.data);
      }
     
    }catch(e){
      //에러시 flag를 달아서 이동
      setError(e);
      if(!lateralDatas) return <div>일치하는 데이터가 없습니다.</div>;
    }
    //로딩 실패시 flag를 달아서 이동
    setLoading(false);
  };
  ...
  if(!lateralDatas) return submitValue();
              ...
              <Clock/>
                <CRow className="searchtoolbar"> 
                  ...
                  <CDataTable
                    ...
                    pagination
                    onPageChange={(index) => {
                      if(index === lateralDatas.length/10){
