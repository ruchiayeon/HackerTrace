...
function Persistence() {
  ...
  const [persistenceDatas, setPersistenceDatas] = useState(false);
    ...
    try{
      ...
      if(pageNumbers>1){
        for(let i =0; i<1000; i++){
          persistenceDatas.push(response.data.data[i])
        }
      }else{
        setPersistenceDatas(response.data.data);
      }
     
    }catch(e){
      //에러시 flag를 달아서 이동
      setError(e);
      if(!persistenceDatas) return <div>일치하는 데이터가 없습니다.</div>;
    }
    //로딩 실패시 flag를 달아서 이동
    setLoading(false);
  };
  ...
  if(!persistenceDatas) return submitValue()
            ...
            <CCardBody>
              <Clock/>
                <CRow className="searchtoolbar"> 
                  ...
                  <CDataTable
                    items={persistenceDatas}
                    fields={fields}
                    itemsPerPage= {10}
                    hover
                    pagination
                    onPageChange={(index) => {
                      if(index === persistenceDatas.length/10){
